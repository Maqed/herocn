...
type BlockViewerContext = {
	...
	tree: FileTree[] | null;
	highlightedFiles: HighlightedFile[] | null;
	iframeKey: number;
...
function BlockViewerToolbar() {
	...
	const [device, setDevice] = React.useState("100%");
		...
		<div className="hidden w-full items-center gap-2 pl-2 md:pr-6 lg:flex">
			...
			<div className="ml-auto flex items-center gap-2">
				<div className="h-8 items-center gap-1.5 rounded-md border p-[3px] shadow-none">
					<ToggleGroup
						...
						onValueChange={(value) => {
							...
							setDevice(next);
							setView("preview");
							resizablePanelRef.current?.resize(next);
						}}
				...
				<Button
					...
					onClick={() => {
						copyToClipboard(
							`npx shadcn@latest add ${getRegistryItemInstallationAlias(item.name)}`,
						);
					}}
				>
					{isCopied ? <Check /> : <Terminal />}
					<span>
						npx shadcn add {getRegistryItemInstallationAlias(item.name)}
					</span>
				</Button>
			</div>
		</div>
...
function BlockViewerIframe({ className }: { className?: string }) {
	...
	return (
		<iframe
			...
			loading="lazy"
			className={`no-scrollbar relative z-20 w-full bg-background ${className ?? ""}`}
		/>
	);
}
	...
	return (
		<div className="flex flex-col gap-2 lg:hidden">
			...
			<div className="overflow-hidden rounded-xl border">
				<BlockViewerIframe className="h-[600px]" />
			</div>
		</div>
	);
}
...
function BlockViewerCode() {
	...
	const language = file.language ?? file.path.split(".").pop() ?? "tsx";

	return (
		<div className="mr-[14px] flex overflow-hidden rounded-xl border bg-surface-secondary text-foreground group-data-[view=preview]/block-view-wrapper:hidden md:h-(--height)">
			<div className="w-72">
				<BlockViewerFileTree />
			</div>
			<figure
			...
			>
				<figcaption
					className="flex h-12 shrink-0 items-center gap-2 border-b px-4 py-2 text-foreground [&_svg]:size-4 [&_svg]:text-foreground [&_svg]:opacity-70"
				...
				<div
					key={file.path}
					className="no-scrollbar overflow-y-auto [&_pre]:bg-transparent!"
				>
...
function BlockViewer({ item, tree, highlightedFiles }: BlockViewerProps) {
	return (
		<BlockViewerProvider
			item={item}
			tree={tree}
