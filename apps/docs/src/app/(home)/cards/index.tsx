import MessageScrollerDemo from "@/registry/new-york-v4/examples/message-scroller-demo";
...
function CardsSkeletonRails() {
	return (
		<div
			aria-hidden="true"
			className="pointer-events-none absolute inset-x-0 top-12 z-10 3xl:block hidden [&_[data-slot=skeleton]:nth-child(even)]:hidden"
		>
			<div className="absolute top-0 left-[calc(50%-960px-var(--rail-width)-var(--gap))] grid w-(--rail-width) grid-cols-[repeat(2,var(--rail-column))] gap-(--gap) opacity-50 [--rail-column:20rem] [--rail-width:calc(var(--rail-column)*2+var(--gap))]">
			...
			<div className="absolute top-0 right-[calc(50%-960px-var(--rail-width)-var(--gap))] grid w-(--rail-width) grid-cols-[repeat(2,var(--rail-column))] gap-(--gap) opacity-50 [--rail-column:20rem] [--rail-width:calc(var(--rail-column)*2+var(--gap))]">
				<div className="flex flex-col gap-(--gap)">
					<SkeletonNewMilestone />
					<SkeletonPayoutThreshold />
					<SkeletonAccountAccess />
...
export function CardsDemo() {
	return (
		<div
			data-slot="demo"
			className="theme-neutral relative flex w-full max-w-none flex-col gap-(--gap) overflow-hidden p-1 pb-0! 3xl:[--gap:--spacing(8)] [--gap:--spacing(8)] md:p-12 lg:p-6 min-[1900px]:p-12 dark:bg-background lg:[--gap:--spacing(6)] min-[1900px]:[--gap:--spacing(10)]!"
		>
			<CardsSkeletonRails />
			<div className="relative z-10 grid grid-cols-1 gap-(--gap) **:data-[slot=card]:w-full md:mx-auto md:max-w-3xl md:grid-cols-2 lg:max-w-none lg:grid-cols-3 xl:max-w-[1600px] 2xl:max-w-[1920px] min-[1400px]:grid-cols-4! min-[1900px]:grid-cols-5!">
