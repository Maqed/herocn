...
function ContextMenuSubTrigger({
	...
	return (
		<ContextMenuPrimitive.SubmenuTrigger
			...
			data-inset={inset}
			className={cn(
				"focus-visible:focus-ring relative flex min-h-9 w-full cursor-default select-none items-center justify-start gap-3 rounded-2xl px-3 py-1.5 text-sm outline-none outline-hidden transition-[color,background-color,box-shadow] data-popup-open:bg-accent data-inset:ps-7 data-popup-open:text-accent-foreground [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
				className,
			)}
			{...props}
		>
...
function ContextMenuSubContent({
	...
	return (
		<ContextMenuContent
			...
			className={cn(
				"relative flex w-full min-w-45 flex-col gap-1 overflow-clip rounded-3xl bg-popover p-1.5 text-popover-foreground shadow-2xl ring-1 ring-foreground/10 md:min-w-55",
				className,
			)}
			side="inline-end"
			{...props}
