							...
							<FieldGroup className="flex flex-row flex-wrap gap-2 rounded-full">
								{options.map((option) => (
									<FieldLabel
										...
										key={option.value}
										className="w-fit! has-[>[data-slot=field]]:border-border"
									>
										<Field
											orientation="horizontal"
											className="gap-1.5 overflow-hidden px-3! py-1.5! transition-all duration-100 ease-linear has-data-checked:px-2!"
										>
											<Checkbox
												value={option.value}
												id={option.value}
